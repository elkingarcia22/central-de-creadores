...
const TestWithoutJoinsPage = () => {
  ...
  const handleTest = async () => {
    setLoading(true);
    try {
      const response = await fetch('/api/google-calendar/test-without-joins', {
        method: 'POST',
  ...
  return (
    <div style={{ padding: '24px', fontFamily: 'Arial, sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <h1>Test sin Joins (Datos Separados)</h1>
        <button
          onClick={handleTest}
          disabled={loading}
          style={{
      ...
      <div style={{ 
        ...
        <p><strong>API:</strong> /api/google-calendar/test-without-joins</p>
        <p><strong>Descripción:</strong> Obtiene datos por separado y los combina manualmente (sin usar joins de Supabase)</p>
      </div>
...
export default TestWithoutJoinsPage;
