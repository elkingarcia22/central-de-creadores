...
export default function CrearParticipanteFriendFamilyModal({
  isOpen,
  onClose,
  onSuccess
}: CrearParticipanteFriendFamilyModalProps) {
  ...
  const [rolesEmpresa, setRolesEmpresa] = useState<any[]>([]);
    ...
    try {
      ...
      const response = await fetch('/api/participantes-friend-family', {
        ...
        body: JSON.stringify({
          ...
          rol_empresa_id: formData.rolEmpresaId || null
        }),
      });
      ...
      if (response.ok) {
        ...
        showError(errorData.error || 'Error al crear el participante');
      }
    } catch (error) {
