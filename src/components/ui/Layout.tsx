...
const Layout: React.FC<LayoutProps> = ({ children, rol, className = '' }) => {
  ...
  const menuConfig = {
    administrador: [
      { label: 'Home', href: '/home', icon: <HomeIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
      { label: 'Investigaciones', href: '/investigaciones', icon: <InvestigacionesIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
      { label: 'Reclutamiento', href: '/reclutamiento', icon: <ReclutamientoIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
      { label: 'Sesiones', href: '/sesiones', icon: <SesionesIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
      { label: 'Métricas', href: '/metricas', icon: <MetricasIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
      { label: 'Participantes', href: '/participantes', icon: <ParticipantesIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
      { label: 'Empresas', href: '/empresas', icon: <EmpresasIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
      { label: 'Conocimiento', href: '/conocimiento', icon: <ConocimientoIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
    ],
    investigador: [
      { label: 'Home', href: '/home', icon: <HomeIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
      { label: 'Investigaciones', href: '/investigaciones', icon: <InvestigacionesIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
  ...
  const utilityItems = esAdministrador() ? [
    { 
      ...
      icon: <ConfiguracionesIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" />,
      subMenu: [
        { label: 'Conexiones', href: '/configuraciones/conexiones', icon: <svg className="w-6 h-6 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg> },
        { label: 'Gestión de Usuarios', href: '/configuraciones/gestion-usuarios', icon: <UsuariosIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
        { label: 'Roles y Permisos', href: '/configuraciones/roles-permisos', icon: <ShieldIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> }
      ]
    },
    { label: 'Sistema de Diseño', href: '/design-system', icon: <DesignSystemIcon className="w-6 h-6 text-gray-500 dark:text-gray-400" /> },
  ] : [];
