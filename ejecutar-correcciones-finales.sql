...
-- 2. Corregir función para actualizar estados existentes
...
DECLARE
    ...
    fecha_sesion_reclutamiento TIMESTAMP WITH TIME ZONE;
...
BEGIN
    ...
    FOR reclutamiento_record IN 
        SELECT r.id, r.fecha_sesion, r.duracion_sesion, r.estado_agendamiento
    ...
    LOOP
        ...
        ELSE
            ...
            fecha_sesion_reclutamiento := reclutamiento_record.fecha_sesion AT TIME ZONE 'America/Bogota';
            duracion_minutos := COALESCE(reclutamiento_record.duracion_sesion, 60);
            
            -- Calcular fecha de fin de sesión
            fecha_fin_sesion := fecha_sesion_reclutamiento + (duracion_minutos || ' minutes')::INTERVAL;
            
            -- Determinar estado basado en fechas
            IF fecha_actual < fecha_sesion_reclutamiento THEN
                nuevo_estado_id := estado_pendiente_id;
            ELSIF fecha_actual >= fecha_sesion_reclutamiento AND fecha_actual <= fecha_fin_sesion THEN
...
BEGIN
    ...
    -- Convertir fecha de sesión a zona horaria de Colombia
    fecha_sesion_reclutamiento := NEW.fecha_sesion AT TIME ZONE 'America/Bogota';
...
-- 5. Ejecutar actualización de estados existentes
