-- ====================================
-- SOLUCIÓN CORRECTA: ESTADOS POR PROGRESO
-- ====================================
-- Ejecutar en Supabase SQL Editor

-- 1. Verificar estados disponibles
SELECT '=== ESTADOS DISPONIBLES ===' as info;
SELECT id, nombre, color, orden, activo FROM estado_reclutamiento_cat WHERE activo = true ORDER BY orden, nombre;

-- 2. Crear estados faltantes si no existen (solo si no están)
SELECT '=== CREAR ESTADOS FALTANTES ===' as info;

-- Crear estado "Pendiente" si no existe
INSERT INTO estado_reclutamiento_cat (id, nombre, color, orden, activo)
SELECT 
    gen_random_uuid(),
    'Pendiente',
    '#F59E0B',
    1,
    true
WHERE NOT EXISTS (SELECT 1 FROM estado_reclutamiento_cat WHERE nombre = 'Pendiente' AND activo = true);
...
SELECT 
    ...
    2,
    true
WHERE NOT EXISTS (SELECT 1 FROM estado_reclutamiento_cat WHERE nombre = 'En progreso' AND activo = true);

-- Crear estado "Agendada" si no existe (NO "Completado")
INSERT INTO estado_reclutamiento_cat (id, nombre, color, orden, activo)
SELECT 
    gen_random_uuid(),
    'Agendada',
    ...
    3,
    true
WHERE NOT EXISTS (SELECT 1 FROM estado_reclutamiento_cat WHERE nombre = 'Agendada' AND activo = true);
...
-- 5. Crear la nueva vista con estado calculado por progreso usando la tabla (CORRECTO)
...
SELECT 
    ...
    -- ESTADO CALCULADO POR PROGRESO USANDO LA TABLA estado_reclutamiento_cat (CORRECTO)
    CASE 
        WHEN COALESCE(ppi.total_participantes, 0) = 0 THEN 
            (SELECT id FROM estado_reclutamiento_cat WHERE nombre = 'Pendiente' AND activo = true)
        WHEN COALESCE(ppi.total_participantes, 0) > 0 AND 
            ...
            (SELECT id FROM estado_reclutamiento_cat WHERE nombre = 'En progreso' AND activo = true)
        WHEN COALESCE(ppi.total_participantes, 0) >= COALESCE(lc.numero_participantes, lc.numero_participantes_esperados, 0) AND
            ...
            (SELECT id FROM estado_reclutamiento_cat WHERE nombre = 'Agendada' AND activo = true)
        ELSE 
            (SELECT id FROM estado_reclutamiento_cat WHERE nombre = 'Pendiente' AND activo = true)
    ...
    CASE 
        ...
        WHEN COALESCE(ppi.total_participantes, 0) >= COALESCE(lc.numero_participantes, lc.numero_participantes_esperados, 0) AND
             COALESCE(lc.numero_participantes, lc.numero_participantes_esperados, 0) > 0 THEN 'Agendada'
        ELSE 'Pendiente'
    END AS estado_reclutamiento_nombre,
    
    CASE 
        WHEN COALESCE(ppi.total_participantes, 0) = 0 THEN 
            (SELECT color FROM estado_reclutamiento_cat WHERE nombre = 'Pendiente' AND activo = true)
        WHEN COALESCE(ppi.total_participantes, 0) > 0 AND 
             COALESCE(ppi.total_participantes, 0) < COALESCE(lc.numero_participantes, lc.numero_participantes_esperados, 0) THEN 
            (SELECT color FROM estado_reclutamiento_cat WHERE nombre = 'En progreso' AND activo = true)
        WHEN COALESCE(ppi.total_participantes, 0) >= COALESCE(lc.numero_participantes, lc.numero_participantes_esperados, 0) AND
             COALESCE(lc.numero_participantes, lc.numero_participantes_esperados, 0) > 0 THEN 
            (SELECT color FROM estado_reclutamiento_cat WHERE nombre = 'Agendada' AND activo = true)
        ELSE 
            (SELECT color FROM estado_reclutamiento_cat WHERE nombre = 'Pendiente' AND activo = true)
...
SELECT '✅ Vista vista_reclutamientos_completa creada con estados por progreso (CORRECTO)' as status;
