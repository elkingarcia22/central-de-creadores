-- Script para verificar datos actuales de paises y actualizarlos con los datos correctos

-- Primero vemos qué datos tiene actualmente la tabla
SELECT 'Datos actuales en la tabla paises:' as mensaje;
SELECT id, nombre, activo FROM paises ORDER BY nombre;

-- Limpiamos la tabla para insertar los datos correctos
DELETE FROM paises;

-- Insertamos los datos correctos del archivo paises_rows.sql
...
-- Verificamos que los datos correctos se insertaron
SELECT 'Datos correctos insertados:' as mensaje;
SELECT COUNT(*) as total_paises FROM paises;

-- Mostramos todos los países correctos
SELECT 'Países correctos en la tabla:' as mensaje;
SELECT id, nombre, activo FROM paises ORDER BY nombre;

-- Aseguramos que RLS esté deshabilitado
ALTER TABLE paises DISABLE ROW LEVEL SECURITY;

-- Verificamos el estado final
SELECT 'Estado final de RLS:' as mensaje;
