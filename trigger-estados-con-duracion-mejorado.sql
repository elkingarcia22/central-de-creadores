-- ====================================
-- TRIGGER MEJORADO PARA ESTADOS CON DURACIÓN
-- ====================================
...
DROP FUNCTION IF EXISTS actualizar_estado_reclutamiento_con_duracion();
...
-- 5. ACTUALIZAR ESTADOS EXISTENTES CON LÓGICA MEJORADA
...
SELECT 
    ...
    NOW() as fecha_actual,
    NOW() AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' as fecha_actual_colombia,
    r.fecha_sesion AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' as fecha_inicio_colombia,
    (r.fecha_sesion + (COALESCE(r.duracion_sesion, 60) || ' minutes')::INTERVAL) AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' as fecha_fin_colombia,
    ea.nombre as estado_actual,
    CASE
        WHEN r.fecha_sesion IS NULL THEN 'Pendiente de agendamiento'
        WHEN NOW() AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' < r.fecha_sesion AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' THEN 'Pendiente'
        WHEN NOW() AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' >= r.fecha_sesion AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' 
             AND NOW() AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' <= (r.fecha_sesion + (COALESCE(r.duracion_sesion, 60) || ' minutes')::INTERVAL) AT TIME ZONE 'UTC' AT TIME ZONE 'America/Bogota' THEN 'En progreso'
        ELSE 'Finalizado'
