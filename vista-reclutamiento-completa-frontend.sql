-- ====================================
-- VISTA RECLUTAMIENTO COMPLETA PARA FRONTEND
-- ====================================
...
-- 2. Crear la nueva vista completa para el frontend
...
SELECT 
    ...
    lc.duracion_estimada AS libreto_duracion,
    lc.duracion_estimada_minutos AS libreto_duracion_minutos,
    lc.objetivos AS libreto_objetivos,
    lc.problema_situacion AS libreto_problema_situacion,
    lc.hipotesis AS libreto_hipotesis,
    lc.resultado_esperado AS libreto_resultado_esperado,
    lc.productos_requeridos AS libreto_productos_requeridos,
    lc.productos_recomendaciones AS libreto_productos_recomendaciones,
    lc.plataforma_id AS libreto_plataforma_id,
    lc.tipo_prueba AS libreto_tipo_prueba,
    lc.tipo_prueba_id AS libreto_tipo_prueba_id,
    lc.rol_empresa_id AS libreto_rol_empresa_id,
    lc.industria_id AS libreto_industria_id,
    lc.pais AS libreto_pais,
    lc.pais_id AS libreto_pais_id,
    lc.usuarios_participantes AS libreto_usuarios_participantes,
    lc.link_prototipo AS libreto_link_prototipo,
    lc.creado_por AS libreto_creado_por,
    lc.creado_el AS libreto_creado_el,
    lc.actualizado_el AS libreto_actualizado_el,
    lc.modalidad_id AS libreto_modalidad_id,
    lc.tamano_empresa_id AS libreto_tamano_empresa_id,
...
LEFT JOIN libretos_completos lc ON iu.libreto_id::uuid = lc.id OR lc.investigacion_id = iu.investigacion_id
...
SELECT 
    '✅ Vista vista_reclutamientos_completa creada completa para frontend' as status;

-- 4. Probar la vista
SELECT 
    '=== PRUEBA DE LA VISTA COMPLETA ===' as info;
