-- ====================================
-- SISTEMA DE LOG DE ACTIVIDADES DE INVESTIGACIONES
-- ====================================
...
-- Trigger para cambios en libreto
CREATE OR REPLACE FUNCTION trigger_cambio_libreto()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF OLD.libreto IS NULL AND NEW.libreto IS NOT NULL THEN
    PERFORM registrar_actividad_investigacion(
      NEW.investigacion_id,
      'creacion_libreto',
      'Libreto creado',
      json_build_object('libreto_id', NEW.id)
    );
  ELSIF OLD.libreto IS DISTINCT FROM NEW.libreto THEN
    PERFORM registrar_actividad_investigacion(
      NEW.investigacion_id,
      'edicion_libreto',
      'Libreto editado',
...
CREATE TRIGGER trigger_cambio_libreto
  AFTER INSERT OR UPDATE ON libretos
  FOR EACH ROW
  EXECUTE FUNCTION trigger_cambio_libreto();

-- Trigger para creación de seguimientos
...
BEGIN
  PERFORM registrar_actividad_investigacion(
    NEW.investigacion_id,
    'creacion_seguimiento',
    'Seguimiento creado: ' || COALESCE(NEW.nombre, 'Sin nombre'),
    json_build_object(
...
CREATE TRIGGER trigger_creacion_seguimiento
  ...
  FOR EACH ROW
  EXECUTE FUNCTION trigger_creacion_seguimiento();

-- Trigger para edición de seguimientos
...
BEGIN
  IF OLD.estado IS DISTINCT FROM NEW.estado THEN
    PERFORM registrar_actividad_investigacion(
      NEW.investigacion_id,
      'edicion_seguimiento',
      'Seguimiento editado: ' || COALESCE(NEW.nombre, 'Sin nombre'),
      json_build_object(
        'seguimiento_id', NEW.id,
        'estado_anterior', OLD.estado,
        'estado_nuevo', NEW.estado
...
CREATE TRIGGER trigger_edicion_seguimiento
  ...
  FOR EACH ROW
  EXECUTE FUNCTION trigger_edicion_seguimiento();
